...
守护进程（daemon）常驻后台收发渠道消息、协调抢答、追踪 agent，但用户**看不到它在做什么**。希望在 macOS 菜单栏（Linux 桌面托盘）提供一个**默认关闭**的小图标，开启后展示运行状态（是否在跑、几个待答、IM 连接、agent 忙闲、有无更新），并能快速打开设置 / 历史 / Agent 状态、检查/应用更新、重启/停止 daemon。
