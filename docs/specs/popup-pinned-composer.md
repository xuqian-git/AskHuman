...
| P12 | 跨题作答 | 固定 Q2 时若用户勾选 Q1 预设选项，视为明确切换作答上下文：清除 Q2 owner；仅滚动、点正文或选 Message 文字不清除 |
...
固定状态不得复用现有 `current` 或 `focusedQ`：

- `current`：当前题 / 快捷键目标，由 scroll-spy、点选和导航维护。
...
- 滚动测量合并到现有 `requestAnimationFrame` 节流，不在每个 `scroll` 事件里反复读写布局。
...
3. 输入框从视口顶部离开时不固定。
4. 纵向多题固定区显示正确题号；scroll-spy 改变 current 不串题；聚焦另一题才切换 owner。
   勾选另一题预设选项会明确结束旧题固定。
...
9. `select-only`、权限确认、提交 / 取消、快捷键和答案输出契约无回归。
