  ...
  src-tauri/                 Rust 后端
    ...
    src/
      ...
      select.rs              通用「单选卡」纯逻辑（传输无关，spec `docs/specs/im-select-card.md`）：
                             ...
                             Watch/Status/Unwatch/Msg（Msg 按钮＝「发送」）。渲染在
...
- **托盘（D7）**：`TrayState.agents`（`TrayAgentInfo` 摘要：seq/kind/标题/项目/状态/待送达/可聚焦；旧 daemon 缺字段→空）驱动「Agent 状态」父项变**子菜单**：首项「打开状态窗口」+ 分隔线 + 每 agent 子菜单（条目文案与 `/status` 列表行同构 `[编号] 类型 — 标题（项目）`、标题截断 24 字符，编号可直接用于 `/msg <编号>`；子项「发送消息…[有待送达时标注]」**仅工作中·非 grok**显示 +「聚焦终端」focusable 时；两者皆无列只读行）；宿主本进程直接开 composer / 后台线程聚焦终端；摘要入 `menu_signature` 走最小 diff。
