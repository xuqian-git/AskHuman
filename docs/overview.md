...
AskHuman/
  ...
  src-tauri/                 Rust 后端
    ...
    src/
      ...
      watch.rs               `/watch` 实时关注纯逻辑：订阅持久化(state/watch.json) + 帧构建/签名 +
                             卡片视图文案(本地化) + 本地时区绝对时刻（见「IM /watch 实时关注」节）
