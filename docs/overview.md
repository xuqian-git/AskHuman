  ...
  src-tauri/                 Rust 后端
    ...
    src/
      ...
      select.rs              通用「单选卡」纯逻辑（传输无关，spec `docs/specs/im-select-card.md`）：
                             ...
                             feishu/card.rs::build_select_card + dingtalk/select.rs（飞书就地回卡 / 钉钉
                             模板+变量、见「IM 单选卡」节）
